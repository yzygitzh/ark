...
struct Constant {
    ...
    static DEVICE DataType lowest() {
        return platform::numeric_limits<DataType>::lowest();
    }
};
...
struct Exp {
    ...
    static DEVICE half compute(half input) { return hexp(input); }
...
struct Sqrt {
    ...
    static DEVICE half compute(half input) { return hsqrt(input); }
    static DEVICE bfloat16 compute(bfloat16 input) {
